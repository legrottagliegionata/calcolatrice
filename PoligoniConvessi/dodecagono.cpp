...
Dodecagono::Dodecagono(const vector<Vertice>&V):PoligonoConvesso(V,12,"Dodecagono"){
      set_lati(V);
}
Dodecagono::Dodecagono(const vector<Vertice>&V,bool b):PoligonoConvesso(V,12,"Dodecagono",b){
      set_lati(V);
}
...
std::ostream& operator<< (std::ostream& os,const Dodecagono& T){
 os<<T.toString();
 return os;
}

void Dodecagono::set_lati(const vector<Vertice>& v){
...
string Dodecagono::toString() const{
  string s ="Dodecagono: ";
